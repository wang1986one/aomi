...
{
	Ogre::RadixSort<Pool<VisualParticle>::PoolList, Particle*, float> ParticleTechnique::mRadixSorter;
	...
	void ParticleTechnique::_markForEmission(void)
	{
		...
		ParticleEmitterList::const_iterator emitterIt;
		ParticleEmitterList::const_iterator emitterItEnd = mEmitters.end();
	...
	void ParticleTechnique::_notifyRescaled(const Ogre::Vector3& scale)
	{
		...
		// Notify the emitters
		ParticleEmitterList::const_iterator itEmitter;
		ParticleEmitterList::const_iterator itEmitterEnd = mEmitters.end();
	...
	{
		assert(emitter && "Emitter is null!");
		ParticleEmitterList::const_iterator it;
		ParticleEmitterList::const_iterator itEnd = mEmitters.end();
		bool notify = false;
	...
	//-----------------------------------------------------------------------
	void ParticleTechnique::_processRenderer(Particle* particle, Ogre::Real timeElapsed, bool firstParticle)
	{
		if (!mRenderer)
			return;
		...
		*/
		...
		if (getParentSystem()->isKeepLocal() || mKeepLocal)
		{
			/** Ignore some rendersystems, because they are rotated by Ogre itself.
				BillboardChain and Entities for example are always rotated (even if not local), based on the node orientation. A Billboard not.
			*/
	...
	bool ParticleTechnique::makeParticleLocal(Particle* particle)
	{
		...
		Ogre::Vector3 diff = getDerivedPosition() - latestPosition; // diff of the technique
	...
	//-----------------------------------------------------------------------
	Ogre::Real ParticleTechnique::getParticleSystemScaleVelocity(void) const
	{
