...
namespace ParticleUniverse
{
	...
	//-------------------------------------------------------------------------
	void TechniqueTranslator::translate(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
		...
		obj->context = Ogre::Any(mTechnique); // Add this to the context, because it is needed for the underlying emitters, affectors, ...
		...
		for(Ogre::AbstractNodeList::iterator i = obj->children.begin(); i != obj->children.end(); ++i)
		{
			...
			else if((*i)->type == Ogre::ANT_OBJECT)
			{
				...
				if (child->cls == token[TOKEN_CAMERA_DEPENDENCY])
				{
					...
					CameraDependency* cameraDependency = OGRE_NEW_T(CameraDependency, Ogre::MEMCATEGORY_SCRIPTING)();
					child->context = Ogre::Any(cameraDependency);
					...
					OGRE_DELETE_T(cameraDependency, CameraDependency, Ogre::MEMCATEGORY_SCRIPTING);
				}
				else
				{
	...
	void ParticleTechniqueWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		if (technique->_getOriginalEnabled() != ParticleTechnique::DEFAULT_ENABLED) serializer->writeLine(
			token[TOKEN_ENABLED], Ogre::StringConverter::toString(technique->_getOriginalEnabled()), 8);
		if (technique->position != ParticleTechnique::DEFAULT_POSITION) serializer->writeLine(
			token[TOKEN_POSITION], Ogre::StringConverter::toString(technique->position), 8);
		if (technique->isKeepLocal() != ParticleTechnique::DEFAULT_KEEP_LOCAL) serializer->writeLine(
			token[TOKEN_KEEP_LOCAL], Ogre::StringConverter::toString(technique->isKeepLocal()), 8);
		if (technique->getVisualParticleQuota() != ParticleTechnique::DEFAULT_VISUAL_PARTICLE_QUOTA) serializer->writeLine(
			token[TOKEN_TECH_VISUAL_PARTICLE_QUOTA], Ogre::StringConverter::toString(technique->getVisualParticleQuota()), 8);
		if (technique->getEmittedEmitterQuota() != ParticleTechnique::DEFAULT_EMITTED_EMITTER_QUOTA) serializer->writeLine(
			token[TOKEN_TECH_EMITTED_EMITTER_QUOTA], Ogre::StringConverter::toString(technique->getEmittedEmitterQuota()), 8);
		if (technique->getEmittedTechniqueQuota() != ParticleTechnique::DEFAULT_EMITTED_TECHNIQUE_QUOTA) serializer->writeLine(
			token[TOKEN_TECH_EMITTED_TECHNIQUE_QUOTA], Ogre::StringConverter::toString(technique->getEmittedTechniqueQuota()), 8);
		if (technique->getEmittedAffectorQuota() != ParticleTechnique::DEFAULT_EMITTED_AFFECTOR_QUOTA) serializer->writeLine(
			token[TOKEN_TECH_EMITTED_AFFECTOR_QUOTA], Ogre::StringConverter::toString(technique->getEmittedAffectorQuota()), 8);
		if (technique->getEmittedSystemQuota() != ParticleTechnique::DEFAULT_EMITTED_SYSTEM_QUOTA) serializer->writeLine(
			token[TOKEN_TECH_EMITTED_SYSTEM_QUOTA], Ogre::StringConverter::toString(technique->getEmittedSystemQuota()), 8);
		...
		if (technique->getLodIndex() != ParticleTechnique::DEFAULT_LOD_INDEX) serializer->writeLine(
			token[TOKEN_TECH_LOD_INDEX], Ogre::StringConverter::toString(technique->getLodIndex()), 8);
		if (technique->getDefaultWidth() != ParticleTechnique::DEFAULT_DEFAULT_WIDTH) serializer->writeLine(
			token[TOKEN_TECH_DEFAULT_PARTICLE_WIDTH], Ogre::StringConverter::toString(technique->getDefaultWidth()), 8);
		if (technique->getDefaultHeight() != ParticleTechnique::DEFAULT_DEFAULT_HEIGHT) serializer->writeLine(
			token[TOKEN_TECH_DEFAULT_PARTICLE_HEIGHT], Ogre::StringConverter::toString(technique->getDefaultHeight()), 8);
		if (technique->getDefaultDepth() != ParticleTechnique::DEFAULT_DEFAULT_DEPTH) serializer->writeLine(
			token[TOKEN_TECH_DEFAULT_PARTICLE_DEPTH], Ogre::StringConverter::toString(technique->getDefaultDepth()), 8);
		if (technique->getSpatialHashingCellDimension() != ParticleTechnique::DEFAULT_SPATIAL_HASHING_CELL_DIM) serializer->writeLine(
			token[TOKEN_TECH_SPHASHING_CELL_DIMENSION], Ogre::StringConverter::toString(technique->getSpatialHashingCellDimension()), 8);
		if (technique->getSpatialHashingCellOverlap() != ParticleTechnique::DEFAULT_SPATIAL_HASHING_CELL_OVERLAP) serializer->writeLine(
			token[TOKEN_TECH_SPHASHING_CELL_OVERLAP], Ogre::StringConverter::toString(technique->getSpatialHashingCellOverlap()), 8);
		if (technique->getSpatialHashTableSize() != ParticleTechnique::DEFAULT_SPATIAL_HASHING_TABLE_SIZE) serializer->writeLine(
			token[TOKEN_TECH_SPHASHING_SIZE], Ogre::StringConverter::toString(technique->getSpatialHashTableSize()), 8);
		if (technique->getSpatialHashingInterval() != ParticleTechnique::DEFAULT_SPATIAL_HASHING_INTERVAL) serializer->writeLine(
			token[TOKEN_TECH_SPHASHING_UPDATE_INTERVAL], Ogre::StringConverter::toString(technique->getSpatialHashingInterval()), 8);
		if (technique->getMaxVelocity() != ParticleTechnique::DEFAULT_MAX_VELOCITY) serializer->writeLine(
			token[TOKEN_TECH_MAX_VELOCITY], Ogre::StringConverter::toString(technique->getMaxVelocity()), 8);
		...
		if (technique->getWidthCameraDependency())
		{
			...
			if (Ogre::Math::Sqrt(technique->getWidthCameraDependency()->getThreshold()) != CameraDependency::DEFAULT_DISTANCE_THRESHOLD) serializer->writeLine(
				token[TOKEN_DISTANCE_THRESHOLD], Ogre::StringConverter::toString(Ogre::Math::Sqrt(technique->getWidthCameraDependency()->getThreshold())), 12);
			if (technique->getWidthCameraDependency()->isIncrease() != CameraDependency::DEFAULT_INCREASE) serializer->writeLine(
				token[TOKEN_INCREASE], Ogre::StringConverter::toString(technique->getWidthCameraDependency()->isIncrease()), 12);
			serializer->writeLine("}", 8);
		}
		...
		if (technique->getHeightCameraDependency())
		{
			...
			serializer->writeLine("{", 8);
			if (Ogre::Math::Sqrt(technique->getHeightCameraDependency()->getThreshold()) != CameraDependency::DEFAULT_DISTANCE_THRESHOLD) serializer->writeLine(
				token[TOKEN_DISTANCE_THRESHOLD], Ogre::StringConverter::toString(Ogre::Math::Sqrt(technique->getHeightCameraDependency()->getThreshold())), 12);
			if (technique->getHeightCameraDependency()->isIncrease() != CameraDependency::DEFAULT_INCREASE) serializer->writeLine(
				token[TOKEN_INCREASE], Ogre::StringConverter::toString(technique->getHeightCameraDependency()->isIncrease()), 12);
			serializer->writeLine("}", 8);
		}
		...
		if (technique->getDepthCameraDependency())
		{
			...
			serializer->writeLine("{", 8);
			if (Ogre::Math::Sqrt(technique->getDepthCameraDependency()->getThreshold()) != CameraDependency::DEFAULT_DISTANCE_THRESHOLD) serializer->writeLine(
				token[TOKEN_DISTANCE_THRESHOLD], Ogre::StringConverter::toString(Ogre::Math::Sqrt(technique->getDepthCameraDependency()->getThreshold())), 12);
			if (technique->getDepthCameraDependency()->isIncrease() != CameraDependency::DEFAULT_INCREASE) serializer->writeLine(
				token[TOKEN_INCREASE], Ogre::StringConverter::toString(technique->getDepthCameraDependency()->isIncrease()), 12);
			serializer->writeLine("}", 8);
		}
