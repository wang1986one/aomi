...
namespace ParticleUniverse
{
	...
	//-----------------------------------------------------------------------
	bool MeshSurfaceEmitterTranslator::translateChildObject(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
		// No objects
		return false;
	}
	...
	void MeshSurfaceEmitterWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		if (emitter->getScale() != MeshSurfaceEmitter::DEFAULT_SCALE) serializer->writeLine(
			token[TOKEN_MESH_SURFACE_MESH_SCALE], Ogre::StringConverter::toString(emitter->getScale()), 12);
		if (emitter->getDistribution() != MeshSurfaceEmitter::DEFAULT_DISTRIBUTION) 
		{
			Ogre::String distribution = token[TOKEN_MESH_SURFACE_HOMOGENEOUS];
