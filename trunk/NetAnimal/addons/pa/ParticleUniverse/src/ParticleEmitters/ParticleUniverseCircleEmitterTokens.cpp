...
namespace ParticleUniverse
{
	...
	//-----------------------------------------------------------------------
	bool CircleEmitterTranslator::translateChildObject(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
		// No objects
		return false;
	}
	...
	void CircleEmitterWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		if (emitter->getRadius() != CircleEmitter::DEFAULT_RADIUS) serializer->writeLine(
			token[TOKEN_RADIUS], Ogre::StringConverter::toString(emitter->getRadius()), 12);
		if (emitter->getStep() != CircleEmitter::DEFAULT_STEP) serializer->writeLine(
			token[TOKEN_STEP], Ogre::StringConverter::toString(emitter->getStep()), 12);
		if (emitter->getCircleAngle() != CircleEmitter::DEFAULT_ANGLE) serializer->writeLine(
			token[TOKEN_ANGLE], Ogre::StringConverter::toString(emitter->getCircleAngle()), 12);
		if (emitter->isRandom() != CircleEmitter::DEFAULT_RANDOM) serializer->writeLine(
			token[TOKEN_EMIT_RANDOM], Ogre::StringConverter::toString(emitter->isRandom()), 12);
		if (emitter->getNormal() != CircleEmitter::DEFAULT_NORMAL) serializer->writeLine(
			token[TOKEN_NORMAL], Ogre::StringConverter::toString(emitter->getNormal()), 12);
