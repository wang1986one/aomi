...
{
	//-----------------------------------------------------------------------
	bool LineEmitterTranslator::translateChildProperty(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
	...
	//-----------------------------------------------------------------------
	bool LineEmitterTranslator::translateChildObject(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
		// No objects
		return false;
	}
	...
	void LineEmitterWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		if (emitter->getEnd() != LineEmitter::DEFAULT_END) serializer->writeLine(
			token[TOKEN_END], Ogre::StringConverter::toString(emitter->getEnd()), 12);
		if (emitter->getMinIncrement() != LineEmitter::DEFAULT_MIN_INCREMENT) serializer->writeLine(
			token[TOKEN_MIN_INCREMENT], Ogre::StringConverter::toString(emitter->getMinIncrement()), 12);
		if (emitter->getMaxIncrement() != LineEmitter::DEFAULT_MAX_INCREMENT) serializer->writeLine(
			token[TOKEN_MAX_INCREMENT], Ogre::StringConverter::toString(emitter->getMaxIncrement()), 12);
		if (emitter->getMaxDeviation() != LineEmitter::DEFAULT_MAX_DEVIATION) serializer->writeLine(
			token[TOKEN_MAX_DEVIATION], Ogre::StringConverter::toString(emitter->getMaxDeviation()), 12);
