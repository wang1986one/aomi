...
namespace ParticleUniverse
{
	...
	//-----------------------------------------------------------------------
	bool RandomiserTranslator::translateChildObject(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
		// No objects
		return false;
	}
	...
	void RandomiserWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		if (affector->getMaxDeviationX() != Randomiser::DEFAULT_MAX_DEVIATION.x) serializer->writeLine(
			token[TOKEN_MAX_DEVIATION_X], Ogre::StringConverter::toString(affector->getMaxDeviationX()), 12);
		if (affector->getMaxDeviationY() != Randomiser::DEFAULT_MAX_DEVIATION.y) serializer->writeLine(
			token[TOKEN_MAX_DEVIATION_Y], Ogre::StringConverter::toString(affector->getMaxDeviationY()), 12);
		if (affector->getMaxDeviationZ() != Randomiser::DEFAULT_MAX_DEVIATION.z) serializer->writeLine(
			token[TOKEN_MAX_DEVIATION_Z], Ogre::StringConverter::toString(affector->getMaxDeviationZ()), 12);
		...
		if (affector->isRandomDirection() != Randomiser::DEFAULT_RANDOM_DIRECTION) serializer->writeLine(
			token[TOKEN_USE_DIRECTION], Ogre::StringConverter::toString(affector->isRandomDirection()), 12);
