...
{
	//-----------------------------------------------------------------------
	bool ColourAffectorTranslator::translateChildProperty(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
		...
		if (prop->name == token[TOKEN_TIME_COLOUR] || prop->name == token[TOKEN_COLOUR_TIME_COLOUR])
		{
			if (passValidatePropertyNumberOfValuesRange(compiler, prop, token[TOKEN_TIME_COLOUR], 4, 5))
			{
				...
				Ogre::AbstractNodeList::const_iterator end = prop->values.end();
				while(i != end)
				{
					float v = 0;
					if(getFloat(*i, &v))
	...
	//-----------------------------------------------------------------------
	bool ColourAffectorTranslator::translateChildObject(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
		// No objects
		return false;
	}
	...
	void ColourAffectorWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		for (it = colourMap.begin(); it != itEnd; ++it)
		{
			time = it->first;
			colour = it->second;
			serializer->writeLine(token[TOKEN_TIME_COLOUR], Ogre::StringConverter::toString(time), Ogre::String(" ") + Ogre::StringConverter::toString(colour), 12);
		}

		if (affector->getColourOperation() != ColourAffector::DEFAULT_COLOUR_OPERATION) 
		{
			Ogre::String colourOperation = token[TOKEN_COLOUR_MULTIPLY];
