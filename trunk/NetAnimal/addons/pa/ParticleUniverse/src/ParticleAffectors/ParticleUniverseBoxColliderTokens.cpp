...
{
	//-----------------------------------------------------------------------
	bool BoxColliderTranslator::translateChildProperty(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
	...
	//-----------------------------------------------------------------------
	bool BoxColliderTranslator::translateChildObject(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
		// No objects
		return false;
	}
	...
	void BoxColliderWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		if (affector->getWidth() != BoxCollider::DEFAULT_WIDTH) serializer->writeLine(
			token[TOKEN_BOX_WIDTH], Ogre::StringConverter::toString(affector->getWidth()), 12);
		if (affector->getHeight() != BoxCollider::DEFAULT_HEIGHT) serializer->writeLine(
			token[TOKEN_BOX_HEIGHT], Ogre::StringConverter::toString(affector->getHeight()), 12);
		if (affector->getDepth() != BoxCollider::DEFAULT_DEPTH) serializer->writeLine(
			token[TOKEN_BOX_DEPTH], Ogre::StringConverter::toString(affector->getDepth()), 12);
