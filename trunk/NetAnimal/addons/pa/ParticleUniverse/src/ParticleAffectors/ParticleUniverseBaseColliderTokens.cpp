...
namespace ParticleUniverse
{
	...
	//-----------------------------------------------------------------------
	bool BaseColliderTranslator::translateChildObject(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
		// No objects
		return false;
	}
	...
	void BaseColliderWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		if (affector->getFriction() != BaseCollider::DEFAULT_FRICTION) serializer->writeLine(
			token[TOKEN_FRICTION], Ogre::StringConverter::toString(affector->getFriction()), 12);
		if (affector->getBouncyness() != BaseCollider::DEFAULT_BOUNCYNESS) serializer->writeLine(
			token[TOKEN_BOUNCYNESS], Ogre::StringConverter::toString(affector->getBouncyness()), 12);
