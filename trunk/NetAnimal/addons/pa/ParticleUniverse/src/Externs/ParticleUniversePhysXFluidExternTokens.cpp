...
#include "ParticleUniversePCH.h"
	...
	{
		//-----------------------------------------------------------------------
		bool PhysXFluidExternTranslator::translateChildProperty(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
		{
		...
		//-----------------------------------------------------------------------
		bool PhysXFluidExternTranslator::translateChildObject(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
		{
			...
			if (child->cls == token[TOKEN_FLAGS])
			{
				...
				child->context = Ogre::Any(externObject);
		...
		//-----------------------------------------------------------------------
		void PhysXFluidFlagTranslator::translate(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
		{
			Ogre::ObjectAbstractNode* obj = reinterpret_cast<Ogre::ObjectAbstractNode*>(node.get());
			PhysXFluidExtern* externObject = Ogre::any_cast<PhysXFluidExtern*>(obj->context);
		...
		void PhysXFluidExternWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
		{
			...
			if (physXFluidExtern->mFluidDesc.restParticlesPerMeter != fluidDesc.restParticlesPerMeter)
				serializer->writeLine(token[TOKEN_REST_PARTICLE_PER_METER], Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.restParticlesPerMeter), 12);

			if (physXFluidExtern->mFluidDesc.restDensity != fluidDesc.restDensity)
				serializer->writeLine(token[TOKEN_REST_DENSITY], Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.restDensity), 12);

			if (physXFluidExtern->mFluidDesc.kernelRadiusMultiplier != fluidDesc.kernelRadiusMultiplier)
				serializer->writeLine(token[TOKEN_KERNEL_RADIUS_MULTIPLIER], Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.kernelRadiusMultiplier), 12);

			if (physXFluidExtern->mFluidDesc.motionLimitMultiplier != fluidDesc.motionLimitMultiplier)
				serializer->writeLine(token[TOKEN_MOTION_LIMIT_MULTIPLIER], Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.motionLimitMultiplier), 12);

			if (physXFluidExtern->mFluidDesc.collisionDistanceMultiplier != fluidDesc.collisionDistanceMultiplier)
				serializer->writeLine(token[TOKEN_COLLISION_DISTANCE_MULTIPLIER], Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.collisionDistanceMultiplier), 12);

			if (physXFluidExtern->mFluidDesc.packetSizeMultiplier != fluidDesc.packetSizeMultiplier)
				serializer->writeLine(token[TOKEN_PACKET_SIZE_MULTIPLIER], Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.packetSizeMultiplier), 12);

			if (physXFluidExtern->mFluidDesc.stiffness != fluidDesc.stiffness)
				serializer->writeLine(token[TOKEN_STIFFNESS], Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.stiffness), 12);

			if (physXFluidExtern->mFluidDesc.viscosity != fluidDesc.viscosity)
				serializer->writeLine(token[TOKEN_VISCOSITY], Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.viscosity), 12);

			if (physXFluidExtern->mFluidDesc.surfaceTension != fluidDesc.surfaceTension)
				serializer->writeLine(token[TOKEN_SURFACE_TENSION], Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.surfaceTension), 12);

			if (physXFluidExtern->mFluidDesc.damping != fluidDesc.damping)
				serializer->writeLine(token[TOKEN_DAMPING], Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.damping), 12);

			if (physXFluidExtern->mFluidDesc.externalAcceleration != fluidDesc.externalAcceleration)
				serializer->writeLine(token[TOKEN_EXTERNAL_ACCELERATION], Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.externalAcceleration.x) + " " + 
				Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.externalAcceleration.y) + " " +
				Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.externalAcceleration.z), 12);

			//////////////////////////////////////////////////////////////////////////
			if (physXFluidExtern->mFluidDesc.restitutionForStaticShapes != fluidDesc.restitutionForStaticShapes)
				serializer->writeLine(token[TOKEN_RESTITUTION_FOR_STATIC_SHAPES], Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.restitutionForStaticShapes), 12);

			if (physXFluidExtern->mFluidDesc.dynamicFrictionForStaticShapes != fluidDesc.dynamicFrictionForStaticShapes)
				serializer->writeLine(token[TOKEN_DYNAMIC_FRICTION_FOR_STATIC_SHAPES], Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.dynamicFrictionForStaticShapes), 12);

			if (physXFluidExtern->mFluidDesc.staticFrictionForStaticShapes != fluidDesc.staticFrictionForStaticShapes)
				serializer->writeLine(token[TOKEN_STATIC_FRICTION_FOR_STATIC_SHAPES], Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.staticFrictionForStaticShapes), 12);

			if (physXFluidExtern->mFluidDesc.attractionForStaticShapes != fluidDesc.attractionForStaticShapes)
				serializer->writeLine(token[TOKEN_ATTRACTION_FOR_STATIC_SHAPES], Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.attractionForStaticShapes), 12);

			//////////////////////////////////////////////////////////////////////////
			if (physXFluidExtern->mFluidDesc.restitutionForDynamicShapes != fluidDesc.restitutionForDynamicShapes)
				serializer->writeLine(token[TOKEN_RESTITUTION_FOR_DYNAMIC_SHAPES], Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.restitutionForDynamicShapes), 12);

			if (physXFluidExtern->mFluidDesc.dynamicFrictionForDynamicShapes != fluidDesc.dynamicFrictionForDynamicShapes)
			serializer->writeLine(token[TOKEN_DYNAMIC_FRICTION_FOR_DYNAMIC_SHAPES], Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.dynamicFrictionForDynamicShapes), 12);

			if (physXFluidExtern->mFluidDesc.staticFrictionForDynamicShapes != fluidDesc.staticFrictionForDynamicShapes)
				serializer->writeLine(token[TOKEN_STATIC_FRICTION_FOR_DYNAMIC_SHAPES], Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.staticFrictionForDynamicShapes), 12);

			if (physXFluidExtern->mFluidDesc.attractionForDynamicShapes != fluidDesc.attractionForDynamicShapes)
			serializer->writeLine(token[TOKEN_ATTRACTION_FOR_DYNAMIC_SHAPES], Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.attractionForDynamicShapes), 12);

			//////////////////////////////////////////////////////////////////////////
			if (physXFluidExtern->mFluidDesc.collisionResponseCoefficient != fluidDesc.collisionResponseCoefficient)
				serializer->writeLine(token[TOKEN_COLLISION_RESPONSE_COEFFICIENT], Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.collisionResponseCoefficient), 12);

			if (physXFluidExtern->mFluidDesc.collisionGroup != fluidDesc.collisionGroup)
				serializer->writeLine(token[TOKEN_COLLISION_GROUP], Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.collisionGroup), 12);
			...
			if (physXFluidExtern->mFluidDesc.groupsMask.bits3 != fluidDesc.groupsMask.bits3 &&
				...
				physXFluidExtern->mFluidDesc.groupsMask.bits0 != fluidDesc.groupsMask.bits0) 
					...
					Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.groupsMask.bits3) + " " +
					Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.groupsMask.bits2) + " " +
					Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.groupsMask.bits1) + " " +
					Ogre::StringConverter::toString(physXFluidExtern->mFluidDesc.groupsMask.bits0), 12);
