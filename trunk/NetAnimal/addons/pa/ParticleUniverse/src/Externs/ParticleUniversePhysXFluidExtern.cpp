	...
	namespace ParticleUniverse
	{
		...
		void PhysXFluidExtern::createFluid(void)
		{
			...
			if (PhysXBridge::getSingletonPtr()->getScene()->getPhysicsSDK().getHWVersion() == NX_HW_VERSION_NONE && (mFluidDesc.flags & NX_FF_HARDWARE))
			{
				...
				Ogre::LogManager::getSingleton().logMessage("ParticleUniverse PhysXFluid: PhysX hardware is not present. Overriding the PhysX Fluid settings to use software mode.");
			}
			...
			if (!mFluidDesc.isValid()) 
			{
				...
				OGRE_EXCEPT(Ogre::Exception::ERR_INVALIDPARAMS, "Invalid PhysX fluid parameters.", "PhysXFluidExtern::createFluid");
			}

			//////////////////////////////////////////////////////////////////////////
			mMaxParticles = static_cast<NxU32>(mParentTechnique->getVisualParticleQuota());

			mPhysXFluidParticles = OGRE_NEW_ARRAY_T(ParticleStructure, mMaxParticles, Ogre::MEMCATEGORY_SCENE_OBJECTS);

			// Initialize the PU particles
			mPUParticles = OGRE_NEW_ARRAY_T(Particle*, mMaxParticles, Ogre::MEMCATEGORY_SCENE_OBJECTS);
			for (NxU32 p=0; p<mMaxParticles; p++)
			{
				mPUParticles[p] = 0;
			}
			...
			mCreatedParticlesID = OGRE_NEW_ARRAY_T(NxU32, mMaxParticles, Ogre::MEMCATEGORY_SCENE_OBJECTS); 

			// Initialize the deletion ID write buffer
			mDeletedParticlesID = OGRE_NEW_ARRAY_T(NxU32, mMaxParticles, Ogre::MEMCATEGORY_SCENE_OBJECTS); 
			...
			if (!mFluid)
			{
				OGRE_EXCEPT(Ogre::Exception::ERR_INTERNAL_ERROR, "PU: Cannot create the PhysX fluid.", "PhysXFluidExtern::createFluid");
			}
		}
		...
		void PhysXFluidExtern::destroyFluid(void)
		{
			...
			if (mPhysXFluidParticles)
			{
				...
				OGRE_DELETE_ARRAY_T(mPhysXFluidParticles, ParticleStructure, mMaxParticles, Ogre::MEMCATEGORY_SCENE_OBJECTS);
				mPhysXFluidParticles = 0;
			}
			...
			{
				// Delete mPhysXFluidParticles
				OGRE_DELETE_ARRAY_T(mCreatedParticlesID, NxU32, mMaxParticles, Ogre::MEMCATEGORY_SCENE_OBJECTS);
				mCreatedParticlesID = 0;
			}
			...
			{
				// Delete mPhysXFluidParticles
				OGRE_DELETE_ARRAY_T(mDeletedParticlesID, NxU32, mMaxParticles, Ogre::MEMCATEGORY_SCENE_OBJECTS);
				mDeletedParticlesID = 0;
			}
			...
			if (mPUParticles)
			{
				...
				::Ogre::CategorisedAllocPolicy<Ogre::MEMCATEGORY_SCENE_OBJECTS>::deallocateBytes((void*)mPUParticles);
				mPUParticles = 0;
			}
		}
		...
		PhysicsActor* PhysXFluidExtern::createPhysXFluidParticle(Particle* particle)
		{
			...
			{	
				...
				if (mNumCreatedParticles != 1) OGRE_EXCEPT(Ogre::Exception::ERR_INVALIDPARAMS, "Particle created, but the number of particles in mCreatedParticlesID is " + mNumCreatedParticles, "PhysXFluidExtern::createPhysicsActor");
		...
		//-----------------------------------------------------------------------
		void PhysXFluidExtern::synchronize(Ogre::Real timeElapsed)
		{
			// Set a flag that we want to update PU particles based on the PhysX particles
			mSynchronize = true;
		}
		//-----------------------------------------------------------------------
		void PhysXFluidExtern::_preProcessParticles(ParticleTechnique* technique, Ogre::Real timeElapsed)
		{
			if (mSynchronize)
			{
				processPhysXDeletionIDBuffer();
		...
		//-----------------------------------------------------------------------
		void PhysXFluidExtern::_postProcessParticles(ParticleTechnique* technique, Ogre::Real timeElapsed)
		{
