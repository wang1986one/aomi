...
{
	//-----------------------------------------------------------------------
	bool BillboardRendererTranslator::translateChildProperty(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
	...
	//-----------------------------------------------------------------------
	bool BillboardRendererTranslator::translateChildObject(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
		// No objects
		return false;
	}
	...
	void BillboardRendererWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		{
			Ogre::String billboardType = token[TOKEN_POINT];
		...
		{
			Ogre::String origin = token[TOKEN_BILLBOARD_CENTER];
		...
		{
			Ogre::String rotationType = token[TOKEN_VERTEX];
		...
		if (renderer->getCommonDirection() != BillboardRenderer::DEFAULT_COMMON_DIRECTION) serializer->writeLine(
			token[TOKEN_BILLBOARD_COMMON_DIRECTION], Ogre::StringConverter::toString(renderer->getCommonDirection()), 12);
		if (renderer->getCommonUpVector() != BillboardRenderer::DEFAULT_COMMON_UP_VECTOR) serializer->writeLine(
			token[TOKEN_BILLBOARD_COMMON_UP_VECTOR], Ogre::StringConverter::toString(renderer->getCommonUpVector()), 12);
		if (renderer->isPointRenderingEnabled() != BillboardRenderer::DEFAULT_POINT_RENDERING) serializer->writeLine(
			token[TOKEN_BILLBOARD_POINT_RENDERING], Ogre::StringConverter::toString(renderer->isPointRenderingEnabled()), 12);
		if (renderer->isUseAccurateFacing() != BillboardRenderer::DEFAULT_ACCURATE_FACING) serializer->writeLine(
			token[TOKEN_BILLBOARD_ACCURATE_FACING], Ogre::StringConverter::toString(renderer->isUseAccurateFacing()), 12);
