...
NoneShowWinner::NoneShowWinner(  boost::shared_ptr<WheelAnimalSceneObj> scene, int winner, const Ogre::Degree & angle):_scene(scene),_winner(winner)
{
...
{
public:
	NodeAllProcess(  boost::shared_ptr<WheelAnimalSceneObj> scene, int winner, const Ogre::Degree & angle,WinEffectPtr effect)
	{
		_first.reset(new NoneShowWinner( scene,winner, angle));
...
{
	return ProcessPtr(new NodeAllProcess( _scene,_winner, Ogre::Degree(static_cast<Ogre::Real>(_rAngle)), _effect));
