...
void WheelAnimalSceneObj::load(const std::string & filename)
{
	...
	{
		//创建摄像机的节点并设置它的位置
		_cameraSn = camera->getParentSceneNode()->createChildSceneNode(Ogre::Vector3(-98.851f, 164.283f, 237.886f));
...
//	std::cout<<_camera->getPosition()<<"!"<<_camera->getOrientation()<<std::endl;
	...
	key->setTranslate(Ogre::Vector3(-98.851f, 164.283f, 237.886f));
	...
	key->setTranslate(Ogre::Vector3(-14.1707f, -120.837f, -1175.47f));
   ...
   //////////////////////////////////
	...
	key = track->createNodeKeyFrame(4);
	key->setTranslate(Ogre::Vector3(0, 300,0));										  
...
int WheelAnimalSceneObj::findRandomAnimal(WheelEnum::ANIMAL_TYPE & type)
{
	int r = rand();
	for(int i = 0 ; i< 24; ++i)
	{
...
Ogre::SceneNode * WheelAnimalSceneObj::getBaseCenter()
{
	if(_sn == NULL)
	{
		...
		for(int i=0 ; i<(BASE_ALL-BASE_0); ++i)
		{
			...
			_sn->addChild(base);
			Ogre::Entity * ent = Orz::OgreGraphicsManager::getSingleton().getSceneManager()->createEntity("ring"+Ogre::StringConverter::toString(i), "zp_dwdzgh.mesh");
			base->createChildSceneNode("ring"+Ogre::StringConverter::toString(i),Ogre::Vector3(0.f, 10.f, 0.f))->attachObject(ent);
