...
AnimalChange::AnimalChange(AnimalPtr animal, Ogre::SceneNode * root)
{
	...
	_sn->scale(Ogre::Vector3(1.0f,1.0f,1.0f)*10);
...
{
	_sn->setPosition(_animals[0]->getSceneNode()->getPosition()+Ogre::Vector3(0.f, -100.f, -100.f));
...
SomethingRise * AnimalChange::getRise(float speed)
{
	...
	if(!_rise && _curAnimal != _animals.end())
		_rise.reset(new SomethingRise(150,speed, (*_curAnimal)->getSceneNode()));
