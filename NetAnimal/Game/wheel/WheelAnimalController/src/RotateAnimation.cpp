...
RotateAnimation::RotateAnimation(const std::string & name,Ogre::SceneNode * sn, Ogre::SceneManager * sm,  bool direction):_node(NULL),_angle(0),_curAngle(0.f),_direction(1),_offsetAngle(0.f)
{
	...
	//
	////Ogre::SceneNode * node = sn;
	//_track = _anim->createNodeTrack(0, sn);

	//// Setup keyframes
	//TransformKeyFrame* key; // startposition
	//
	//Ogre::Quaternion q;

	//key = _track->createNodeKeyFrame(0);
	//key->setTranslate(sn->getPosition());
	//
	//q.FromAngleAxis(Ogre::Degree(direction?0.f:-0.f), Ogre::Vector3::UNIT_Y);
	//key->setRotation(q);
	//
	//key = _track->createNodeKeyFrame(90);
	//key->setTranslate(sn->getPosition());
	//
	//q.FromAngleAxis(Ogre::Degree(direction?90.f:-90.f), Ogre::Vector3::UNIT_Y);
	//key->setRotation(q);
	//
	//key->setTranslate(sn->getPosition());

	//key = _track->createNodeKeyFrame(180.f);
	//q.FromAngleAxis(Ogre::Degree(direction?180.f:-180.f), Ogre::Vector3::UNIT_Y);
	//key->setRotation(q);
	...
	//key = _track->createNodeKeyFrame(270.f);
	//q.FromAngleAxis(Ogre::Degree(direction?270.f:-270.f), Ogre::Vector3::UNIT_Y);
	//key->setRotation(q);

	//key->setTranslate(sn->getPosition());


	//key = _track->createNodeKeyFrame(360.f);
	//q.FromAngleAxis(Ogre::Degree(direction?360.f:-360.f), Ogre::Vector3::UNIT_Y);
	//key->setRotation(q);
	//
	//key->setTranslate(sn->getPosition());
