...
template<> NewHardwareCode* Singleton<NewHardwareCode>::_singleton = NULL;
...
void NewHardwareCode::update(void)
{
	HardwareMsg msg;
...
void NewHardwareCode::decode(const boost::array<unsigned char , 40> & container, HardwareMsg & receive)
{
...
void NewHardwareCode::_threadRunning(void)
{
	EncodeContainer in;
	Container code;
	HardwareMsg receive;
...
void NewHardwareCode::push_decode(const Container & container)
{
	_decodeQueue.enqueue(container);
}
NewHardwareCode::~NewHardwareCode(void)
{
...
NewHardwareCode::NewHardwareCode(void)
{
	...
	_isRunning = true;
	_thrd.reset(new ::boost::thread(boost::bind(&NewHardwareCode::_threadRunning, this)));
...
void NewHardwareCode::encode(unsigned char order, unsigned char bTime, boost::array<unsigned char , 40> & container)
{
	EncodeContainer in;
	in[0] = order;
	in[1] = bTime;
...
void NewHardwareCode::setDataSize(WheelEnum::HARDWARE data, size_t size)
{
	_dataSize[data] = size;
}
