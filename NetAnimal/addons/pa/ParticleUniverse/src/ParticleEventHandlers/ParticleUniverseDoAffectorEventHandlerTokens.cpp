...
{
	//-----------------------------------------------------------------------
	bool DoAffectorEventHandlerTranslator::translateChildProperty(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
	...
	//-----------------------------------------------------------------------
	bool DoAffectorEventHandlerTranslator::translateChildObject(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
		// No objects
		return false;
	}
	...
	void DoAffectorEventHandlerWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		if (eventHandler->getPrePost() != DoAffectorEventHandler::DEFAULT_PRE_POST) serializer->writeLine(
			token[TOKEN_FORCE_AFFECTOR_PRE_POST], Ogre::StringConverter::toString(eventHandler->getPrePost()), 16);
