...
{
	//-----------------------------------------------------------------------
	bool OnEventFlagObserverTranslator::translateChildProperty(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
	...
	//-----------------------------------------------------------------------
	bool OnEventFlagObserverTranslator::translateChildObject(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
		// No objects
		return false;
	}
	...
	void OnEventFlagObserverWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		if (observer->getEventFlag() != OnEventFlagObserver::DEFAULT_EVENT_FLAG) serializer->writeLine(
			token[TOKEN_ONEVENT_FLAG], Ogre::StringConverter::toString(observer->getEventFlag()), 12);
