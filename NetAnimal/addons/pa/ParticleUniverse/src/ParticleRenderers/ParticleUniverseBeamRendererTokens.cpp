...
{
	//-----------------------------------------------------------------------
	bool BeamRendererTranslator::translateChildProperty(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
	...
	//-----------------------------------------------------------------------
	bool BeamRendererTranslator::translateChildObject(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
		// No objects
		return false;
	}
	...
	void BeamRendererWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		if (renderer->getUpdateInterval() != BeamRenderer::DEFAULT_UPDATE_INTERVAL) serializer->writeLine(
			token[TOKEN_UPDATE_INTERVAL], Ogre::StringConverter::toString(renderer->getUpdateInterval()), 12);
		if (renderer->getDeviation() != BeamRenderer::DEFAULT_DEVIATION) serializer->writeLine(
			token[TOKEN_BEAMRENDERER_DEVIATION], Ogre::StringConverter::toString(renderer->getDeviation()), 12);
		if (renderer->getNumberOfSegments() != BeamRenderer::DEFAULT_NUMBER_OF_SEGMENTS) serializer->writeLine(
			token[TOKEN_NUMBER_OF_SEGMENTS], Ogre::StringConverter::toString(renderer->getNumberOfSegments()), 12);
		if (renderer->isJump() != false) serializer->writeLine(
			token[TOKEN_BEAMRENDERER_JUMP], Ogre::StringConverter::toString(renderer->isJump()), 12);
