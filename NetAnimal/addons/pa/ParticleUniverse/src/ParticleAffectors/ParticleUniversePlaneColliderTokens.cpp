...
{
	//-----------------------------------------------------------------------
	bool PlaneColliderTranslator::translateChildProperty(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
	...
	//-----------------------------------------------------------------------
	bool PlaneColliderTranslator::translateChildObject(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
		// No objects
		return false;
	}
	...
	void PlaneColliderWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		if (affector->getNormal() != PlaneCollider::DEFAULT_NORMAL) serializer->writeLine(
			token[TOKEN_NORMAL], Ogre::StringConverter::toString(affector->getNormal()), 12);
