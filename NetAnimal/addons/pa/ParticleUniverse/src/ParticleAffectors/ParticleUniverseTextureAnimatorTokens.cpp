...
{
	//-----------------------------------------------------------------------
	bool TextureAnimatorTranslator::translateChildProperty(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
	...
	//-----------------------------------------------------------------------
	bool TextureAnimatorTranslator::translateChildObject(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
		// No objects
		return false;
	}
	...
	void TextureAnimatorWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		if (affector->getAnimationTimeStep() != TextureAnimator::DEFAULT_TIME_STEP) serializer->writeLine(
			token[TOKEN_TIME_STEP], Ogre::StringConverter::toString(affector->getAnimationTimeStep()), 12);
		if (affector->getTextureCoordsStart() != TextureAnimator::DEFAULT_TEXCOORDS_START) serializer->writeLine(
			token[TOKEN_START_TEXANIM_TEXCOORDS_RANGE], Ogre::StringConverter::toString(affector->getTextureCoordsStart()), 12);
		if (affector->getTextureCoordsEnd() != TextureAnimator::DEFAULT_TEXCOORDS_END) serializer->writeLine(
			token[TOKEN_END_TEXANIM_TEXCOORDS_RANGE], Ogre::StringConverter::toString(affector->getTextureCoordsEnd()), 12);
		if (affector->isStartRandom() != TextureAnimator::DEFAULT_START_RANDOM) serializer->writeLine(
			token[TOKEN_TEXANIM_START_RANDOM], Ogre::StringConverter::toString(affector->isStartRandom()), 12);

		if (affector->getTextureAnimationType() != TextureAnimator::DEFAULT_ANIMATION_TYPE) 
		{
			Ogre::String animationType = token[TOKEN_TEXANIM_LOOP];
