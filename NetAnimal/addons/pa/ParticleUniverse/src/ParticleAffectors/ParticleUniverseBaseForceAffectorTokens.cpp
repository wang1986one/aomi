...
{
	//-----------------------------------------------------------------------
	bool BaseForceAffectorTranslator::translateChildProperty(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
	...
	//-----------------------------------------------------------------------
	bool BaseForceAffectorTranslator::translateChildObject(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
		// No objects
		return false;
	}
	...
	void BaseForceAffectorWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		if (affector->getForceVector() != BaseForceAffector::DEFAULT_FORCE_VECTOR) serializer->writeLine(
			token[TOKEN_FORCE_VECTOR], Ogre::StringConverter::toString(affector->getForceVector()), 12);

		if (affector->getForceApplication() != BaseForceAffector::DEFAULT_FORCE_APPL) 
		{
			Ogre::String application = token[TOKEN_FORCE_ADD];
