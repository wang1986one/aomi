...
{
	//-----------------------------------------------------------------------
	bool InterParticleColliderTranslator::translateChildProperty(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
	...
	//-----------------------------------------------------------------------
	bool InterParticleColliderTranslator::translateChildObject(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
		// No objects
		return false;
	}
	...
	void InterParticleColliderWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		if (affector->getAdjustment() != InterParticleCollider::DEFAULT_ADJUSTMENT) serializer->writeLine(
			token[TOKEN_ADJUSTMENT], Ogre::StringConverter::toString(affector->getAdjustment()), 12);

		if (affector->getInterParticleCollisionResponse() != InterParticleCollider::DEFAULT_COLLISION_RESPONSE) 
		{
			Ogre::String collisionResponse = token[TOKEN_IPC_ANGLE_BASED_VELOCITY];
