...
{
	//-----------------------------------------------------------------------
	bool PathFollowerTranslator::translateChildProperty(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
	...
	//-----------------------------------------------------------------------
	bool PathFollowerTranslator::translateChildObject(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
		// No objects
		return false;
	}
	...
	void PathFollowerWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		if (numberOfPoints > 0)
		{
			...
			{
				serializer->writeLine(token[TOKEN_PATH_POINT], Ogre::StringConverter::toString(affector->getPoint(u)), 12);
			}
		}
