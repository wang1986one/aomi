...
{
	//-----------------------------------------------------------------------
	bool GeometryRotatorTranslator::translateChildProperty(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
	...
	//-----------------------------------------------------------------------
	bool GeometryRotatorTranslator::translateChildObject(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
	...
	void GeometryRotatorWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		if (affector->useOwnRotationSpeed() != GeometryRotator::DEFAULT_USE_OWN) serializer->writeLine(
			token[TOKEN_USE_OWN_ROTATION], Ogre::StringConverter::toString(affector->useOwnRotationSpeed()), 12);
		if (affector->getRotationAxis() != GeometryRotator::DEFAULT_ROTATION_AXIS) serializer->writeLine(
			token[TOKEN_ROTATION_AXIS], Ogre::StringConverter::toString(affector->getRotationAxis()), 12);
