...
namespace ParticleUniverse
{
	...
	//-----------------------------------------------------------------------
	bool BoxEmitterTranslator::translateChildObject(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
		// No objects
		return false;
	}
	...
	void BoxEmitterWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		if (emitter->getWidth() != BoxEmitter::DEFAULT_WIDTH) serializer->writeLine(
			token[TOKEN_BOX_WIDTH], Ogre::StringConverter::toString(emitter->getWidth()), 12);
		if (emitter->getHeight() != BoxEmitter::DEFAULT_HEIGHT) serializer->writeLine(
			token[TOKEN_BOX_HEIGHT], Ogre::StringConverter::toString(emitter->getHeight()), 12);
		if (emitter->getDepth() != BoxEmitter::DEFAULT_DEPTH) serializer->writeLine(
			token[TOKEN_BOX_DEPTH], Ogre::StringConverter::toString(emitter->getDepth()), 12);
