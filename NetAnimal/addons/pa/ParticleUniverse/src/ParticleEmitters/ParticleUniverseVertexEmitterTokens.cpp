...
{
	//-----------------------------------------------------------------------
	bool VertexEmitterTranslator::translateChildProperty(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
	...
	//-----------------------------------------------------------------------
	bool VertexEmitterTranslator::translateChildObject(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
		// No objects
		return false;
	}
	...
	void VertexEmitterWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		if (emitter->getSegments() != VertexEmitter::DEFAULT_SEGMENTS) serializer->writeLine(
			token[TOKEN_NUMBER_OF_SEGMENTS], Ogre::StringConverter::toString(emitter->getSegments()), 12);
		if (emitter->getIterations() != VertexEmitter::DEFAULT_ITERATIONS) serializer->writeLine(
			token[TOKEN_ITERATIONS], Ogre::StringConverter::toString(emitter->getIterations()), 12);
