...
namespace ParticleUniverse
{
	...
	//-----------------------------------------------------------------------
	bool SceneDecoratorExternTranslator::translateChildObject(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
		// No objects
		return false;
	}
	...
	void SceneDecoratorExternWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		if (sceneDecoratorExtern->getScale() != Ogre::Vector3::UNIT_SCALE) serializer->writeLine(
			token[TOKEN_SCENE_SCALE], Ogre::StringConverter::toString(sceneDecoratorExtern->getScale()), 12);
		if (sceneDecoratorExtern->getPosition() != Ogre::Vector3::ZERO) serializer->writeLine(
			token[TOKEN_SCENE_POSITION], Ogre::StringConverter::toString(sceneDecoratorExtern->getPosition()), 12);
