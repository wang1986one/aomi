...
{
	//-----------------------------------------------------------------------
	bool GravityExternTranslator::translateChildProperty(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
	...
	//-----------------------------------------------------------------------
	bool GravityExternTranslator::translateChildObject(Ogre::ScriptCompiler* compiler, const Ogre::AbstractNodePtr &node)
	{
		// No objects
		return false;
	}
	...
	void GravityExternWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		if (gravityExtern->getGravity() != GravityAffector::DEFAULT_GRAVITY) serializer->writeLine(
			token[TOKEN_GRAVITY], Ogre::StringConverter::toString(gravityExtern->getGravity()), 12);
